...
#include <random> // для лучшей генерации чисел

using namespace std;

// быстрая сортировка (quicksort)
void quicksort(int arr[], int l, int h) {
    // ОПТИМИЗАЦИЯ: заменяем хвостовую рекурсию циклом
    while (l < h) {
        ...
        // ОПТИМИЗАЦИЯ: рекурсивно сортируем меньшую часть, а большую обрабатываем в цикле
        if (highInd - l < h - lowInd) {
            // левая часть меньше - рекурсивно сортируем её
            if (highInd > l) {
                quicksort(arr, l, highInd); // сортируем левую часть
            }
            l = lowInd; // продолжаем с правой частью в цикле
        }
        else {
            ...
            h = highInd; // продолжаем с левой частью в цикле
        }
    }
}
...
// Функция для проверки сортировки (опционально)
...
int main() {
    ...
    // УЛУЧШЕНИЕ: используем более равномерное распределение чисел
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> dis(0, 1000000); // больший диапазон чисел

    // заполнение массива случайными числами
    for (int i = 0; i < size; i++) {
        arr[i] = dis(gen); // случайные числа от 0 до 1 млн
    }

    cout << "Сортировка " << size << " элементов...\n";
