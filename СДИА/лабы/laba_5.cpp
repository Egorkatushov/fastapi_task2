...
Node* maxValueNode(Node* node) { // функция поиска максимального узла в поддереве
    Node* current = node;
    while (current->right != nullptr) // идем вправо до упора
        current = current->right;

    return current; // возвращаем самый правый узел
}
...
Node* deleteNode(Node* root, int key) { // функция удаления узла
    ...
    if (key < root->key) // ищем ключ в левом поддереве
        ...
        if ((root->left == nullptr) || (root->right == nullptr)) { // случай 0 или 1 ребенок
            ...
            // Используем максимальный узел из ЛЕВОГО поддерева
            Node* temp = maxValueNode(root->left); // ищем максимальный в левом поддереве
            root->key = temp->key; // копируем его значение
            root->left = deleteNode(root->left, temp->key); // удаляем этот узел из левого поддерева
        }
    }
...
int main() {
    ...
    // Удаление двух элементов подряд
    for (int i = 0; i < 2; i++) {
        int d;
        cout << "\nВведите ключ для удаления (удаление " << i + 1 << "-е): ";
        cin >> d;

        root = deleteNode(root, d);

        cout << "После " << i + 1 << "-го удаления:" << endl;
        print(root, k);
